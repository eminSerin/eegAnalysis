...
dataExt = {'org','toICA'}; % output data names. 
...
for i = 1:nfile
    ...
    TMP = EEG; % copy to temp file for further analysis.
    ...
    for step = 1:2
        ...
        if step == 1
            ...
            % 1 - 35 Butterworth Filter for ICA.
            EEG  = pop_basicfilter( EEG,  1:64 , 'Boundary', 'boundary', 'Cutoff',...
                [1 35], 'Design', 'butter', 'Filter', 'bandpass', 'Order',4 );
        end
        ...
        if step == 1
            ...
            % Reject epochs.
            EEG = pop_select(EEG,'notrial',rejIndex);
        end
        
        %% Save preICA dataset.
        pop_saveset( EEG, 'filename', [cDataName, '_preICA_' dataExt{step}], 'filepath', eegpath);
        close all;
        
        EEG = TMP; % change current eeg structure with old one.
